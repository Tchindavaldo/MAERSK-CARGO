...
CREATE TABLE IF NOT EXISTS jongleur_maersk_site_settings (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  site_email text DEFAULT '',
  site_phone text DEFAULT '',
  site_address text DEFAULT '',
  support_email text DEFAULT '',
  company_name text DEFAULT 'Jongleur Maersk',
...
ALTER TABLE jongleur_maersk_site_settings ENABLE ROW LEVEL SECURITY;

-- Anyone can read site settings
CREATE POLICY "Anyone can view site settings"
  ON jongleur_maersk_site_settings FOR SELECT
  TO anon, authenticated
  USING (true);

-- Only authenticated users can update site settings
CREATE POLICY "Authenticated users can update site settings"
  ON jongleur_maersk_site_settings FOR UPDATE
  TO authenticated
  USING (true)
  WITH CHECK (true);
...
CREATE POLICY "Authenticated users can insert site settings"
  ON jongleur_maersk_site_settings FOR INSERT
  TO authenticated
  WITH CHECK (true);
